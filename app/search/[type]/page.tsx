...
const page = ({ params }: { params: { type: string } }) => {
  ...
  const fetchSearchData = (delayRate: number) => {
    ...
    const fetchdata = async () => {
      const response: Batch | null = await FetchAnime(`/${params.type}`, delayRate, parameters, signal);
      setPaginationData(prevState => (response as Batch) ? (response as Batch).pagination : prevState)
      setData(prevState => (response as Batch) ? (response as Batch).data : prevState);
      setIsLoading(false)
    };
  ...
  const fetchGenre = async () => {
    const response: Genre | null = await FetchAnime(`/genres/${params.type}`, 0);
        ...
        >
          <div className="content relative z-20 w-full pt-24 px-6 tablet:px-8 tablet:pt-[8.25rem] desktop:flex desktop:px-16 desktop:pt-[10.25rem]">
            ...
            <div className="right w-full relative">
              <InputBox type={params.type} value={query} setValue={setQuery} controller={controllerRef}/>
              <FilterBadge genreActiveFilter={genre} genreFilter={genreFilters} mangaFilter={mangaStatusFilters} resultLength={paginationData.items.total} rating={rating} type={type}/>         
              <SearchCard data={data} isLoading={isLoading} type={params.type}/>
            </div>
          </div>
